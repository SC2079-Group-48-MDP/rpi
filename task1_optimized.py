...
class RaspberryPi:
    ...
    def snap_and_rec(self, obstacle_id_with_signal: str) -> None:
        """
        RPi snaps an image and calls the API for image-rec asynchronously.
        ...
        # Use a thread pool to handle image capture and API request
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(self._capture_and_send_image, obstacle_id, signal)
...
def _capture_and_send_image(self, obstacle_id: str, signal: str) -> dict:
    """
    Captures the image and sends it to the image recognition API.
    ...
    # Read configuration
    config_file = "/home/" + os.getlogin() + "/PiLCConfig9.txt"
    with open(config_file, "r") as file:
        config = list(map(int, file.read().splitlines()))
    
    # Extract configuration parameters
    mode, speed, gain, brightness, contrast, _, red, blue, ev, _, meter, awb, sharpness, denoise, _, extn, saturation, quality = (
        config[i] for i in [0, 1, 2, 3, 4, 6, 7, 8, 15, 19, 20, 21, 22, 23, 24]
    )
    ...
    while retry_count < max_retries:
        ...
        shutter = abs(1/shutter) if shutter < 0 else shutter
        sspeed = int(shutter * 1000000)
        sspeed += 1 if (shutter * 1000000) - int(shutter * 1000000) > 0.5 else 0
        ...
        if sspeed > 1000000 and mode == 0:
            ...
            if awb == 0:
                rpistr += ["--awbgains", f"{red/10},{blue/10}"]
            else:
                rpistr += ["--awb", awbs[awb]]

        # Execute the command
        result = subprocess.run(rpistr, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if result.returncode != 0:
            self.logger.error(f"libcamera-still failed: {result.stderr.decode()}")
            continue
        ...
        # Check results and decide whether to retry
        if results.get('image_id') != 'NA' or retry_count > 6:
            break
        elif retry_count > 3:
            self.logger.info(f"Image recognition results: {results}")
            self.logger.info("Recapturing with lower shutter speed...")
            speed = max(speed - 1, 0)  # Ensure speed doesn't go below 0
        else:
            self.logger.info(f"Image recognition results: {results}")
            self.logger.info("Recapturing with higher shutter speed...")
            speed = min(speed + 1, len(shutters) - 1)  # Ensure speed doesn't exceed list
